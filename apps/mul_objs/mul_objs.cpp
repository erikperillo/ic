...
int readConf(Args& args,const char* filename,bool debug)
{
	char line[BUF_SIZE],*word,*line_args,garbage[100],obj_str[256],path[BUF_SIZE] = "",temp[BUF_SIZE]; 
	const char *delim = "	 \n";
	...
	{	
		...
		else if(strcmp(word,"songs_upl:")==0)
		{
			...
			{
				sprintf(temp,"%s/%s",path,word);
				if(sscanf(temp,"%s",args.songs[obj_count-1][0][i]) != 1)
					return -1;
			}
			if(i != args.nsongs)
				return -1;
		...
		else if(strcmp(word,"songs_upr:")==0)
		{
			...
			{
				sprintf(temp,"%s/%s",path,word);
				if(sscanf(temp,"%s",args.songs[obj_count-1][1][i]) != 1)
					return -1;
			}
			if(i != args.nsongs)
				return -1;
		...
		else if(strcmp(word,"songs_btl:")==0)
		{
			...
			{
				sprintf(temp,"%s/%s",path,word);
				if(sscanf(temp,"%s",args.songs[obj_count-1][2][i]) != 1)
					return -1;
			}
			if(i != args.nsongs)
				return -1;
		...
		else if(strcmp(word,"songs_btr:")==0)
		{
			...
			{
				sprintf(temp,"%s/%s",path,word);
				if(sscanf(temp,"%s",args.songs[obj_count-1][3][i]) != 1)
					return -1;
			}
			if(i != args.nsongs)
				return -1;
	...
	if(debug)
	{
		cout << "ReadConf: numbers:" << endl;
		cout << "nargs = " << nargs << endl;
		cout << "nobjs = " << args.nobjs << endl;
		cout << "nsongs = " << args.nsongs << endl;
		cout << "args.nobjs*(2+args.nsongs*4) = " << args.nobjs*(2+args.nsongs*4) << endl;
	}
...
int main(int argc, char** argv)
{
	...
	bool debug = container.parse<bool>(clarg::Clarg("-d --debug","display aditional informatil helpful for debugging"));
